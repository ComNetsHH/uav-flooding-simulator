...
def get_result(v, num_nodes, run, interval = 1000, forwardingProbability=100):
    with open(f'res/v{v}_parsed/sf_n{num_nodes}_i{interval}_p{forwardingProbability}_r{run}.json') as f:
        data = json.load(f)
        return data


def main(interval= 300, forwardingProbability= 100):
    v = 7
    ...
    for num_nodes in nn:
        ...
        for r in range(0,11):
            try:
                data = get_result(v, num_nodes, r, interval, forwardingProbability)
    ...
    plt.title(f'Avg AoI & Dissemination Rate (SF) i={interval}ms, p={forwardingProbability}%')

    fig.tight_layout()  # otherwise the right y-label is slightly clipped
    fig.savefig(f'./figures/sf_aoi_diss_rate_i{interval}_p{forwardingProbability}_v{v}.png', dpi=500, bbox_inches='tight', pad_inches=0.01)
    fig.savefig(f'./figures/sf_aoi_diss_rate_i{interval}_p{forwardingProbability}_v{v}.pdf', dpi=500, bbox_inches='tight', pad_inches=0.01)
    #plt.show()
