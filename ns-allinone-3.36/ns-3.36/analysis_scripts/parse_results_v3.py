...
def extract_stats(data, v, file_name, stats={}):
    ...
    with open(f'./res/v{v}_parsed/summary_{file_name}.dat', 'wb') as f:
...
def extract_num_forwarded_per_user_dist(data, v, file_name):
    ...
    with open(f'./res/v{v}_parsed/{file_name}_fwd_by_user_dist.dat', 'wb') as f:
        marshal.dump(forwarded_events_by_user.tolist(), f)

def extract_num_received_per_user_dist(data, v, file_name):
    rcvd_events = data[(data['eventType'] == 'PktRcvd')]
    rcvd_events_by_user = rcvd_events.groupby('nodeId').size().values
    with open(f'./res/v{v}_parsed/{file_name}_rcvd_by_user_dist.dat', 'wb') as f:
        marshal.dump(rcvd_events_by_user.tolist(), f)
...
def extract_num_forwarded_per_seqno_dist(data, v, file_name):
    ...
    with open(f'./res/v{v}_parsed/{file_name}_fwd_by_user_seqno_dist.dat', 'wb') as f:
        marshal.dump(forwarded_events_by_user.tolist(), f)

def extract_num_received_per_seqno_dist(data, v, file_name):
    rcvd_events = data[(data['eventType'] == 'PktRcvd')]
    rcvd_events_by_user = rcvd_events.groupby('seqNo').size().values
    with open(f'./res/v{v}_parsed/{file_name}_rcvd_by_seqno_dist.dat', 'wb') as f:
        marshal.dump(rcvd_events_by_user.tolist(), f)
...
def extract_aoi_distribution(data, v, file_name):
    ...
    with open(f'./res/v{v}_parsed/{file_name}_aoi_dist.dat', 'wb') as f:
        marshal.dump(data, f)
...
def get_peaks(ts):
    ...
    for i in range(1, len(ts)-1):
        ...
        if (current_aoi > prev_aoi) and (current_aoi > next_aoi):
            peaks.append(current_aoi)
...
def extract_aoi_distribution_by_max_dist(data, v, file_name, positions, R):
    ...
    excess_probability_1_hop_dist = 1 - np.sum(p[t <= t_max])
    excess_probability_1_hop_peak = len(peaks[peaks >= t_max]) / len(peaks)

    with open(f'./res/v{v}_parsed/{file_name}_aoi_dist_r{int(R/COMM_RANGE)}.dat', 'wb') as f:
        marshal.dump(data, f)
            
    return (excess_probability_1_hop_dist, excess_probability_1_hop_peak)
...
def extract_aoi_distribution_by_max_hop(data, v, file_name, maxHops):
    ...
    with open(f'./res/v{v}_parsed/{file_name}_aoi_dist_h{maxHops}.dat', 'wb') as f:
        marshal.dump(data, f)
...
def main(v, file_name):
    ...
    (excess_probability_1_hop_dist, excess_probability_1_hop_peak) = extract_aoi_distribution_by_max_dist(data, v, file_name, positions, COMM_RANGE)
    ...
    stats = {
        ...
        'excess_probability_1_R': excess_probability_1_hop_dist,
        'excess_probability_1_R_dist': excess_probability_1_hop_dist,
        'excess_probability_1_R_peak': excess_probability_1_hop_peak
    }
