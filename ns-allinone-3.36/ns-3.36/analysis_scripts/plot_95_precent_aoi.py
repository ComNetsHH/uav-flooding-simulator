...
def main(interval=50, decayFactor=100):
    v = 13
    ...
    for num_nodes in nn:
        ...
        for r in range(0,10):
            try:
                ...
                aoi.append(data['aoi_95_percentile']) #avg_dissemination_rate
    ...
    ax1.set_ylabel('AoI [ms]', color=color)
    ...
    fig.savefig(f'./figures/rdf_95_percent_aoi_i{interval}_q{decayFactor}_v{v}.png', dpi=500, bbox_inches='tight', pad_inches=0.01)
    fig.savefig(f'./figures/rdf_95_percent_aoi_i{interval}_q{decayFactor}_v{v}.pdf', dpi=500, bbox_inches='tight', pad_inches=0.01)
    plt.show()
