...
def get_result(v, num_nodes, run, interval = 1000):
    with open(f'res/v{v}_parsed/pf_n{num_nodes}_i{interval}_r{run}.json') as f:
        data = json.load(f)
        return data
...
def main(interval=1000):
    ...
    plt.title(f'Avg AoI & Dissemination Rate (PF) i={interval}ms')

    fig.tight_layout()  # otherwise the right y-label is slightly clipped
    fig.savefig(f'./figures/pf_aoi_diss_rate_i{interval}_v{v}.png', dpi=500, bbox_inches='tight', pad_inches=0.01)
    fig.savefig(f'./figures/pf_aoi_diss_rate_i{interval}_v{v}.pdf', dpi=500, bbox_inches='tight', pad_inches=0.01)
    #plt.show()
