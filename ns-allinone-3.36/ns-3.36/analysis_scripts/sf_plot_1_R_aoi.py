...
def get_result(v, num_nodes, run, interval = 1000, p=100):
    with open(f'res/v{v}_parsed/summary_sf_n{num_nodes}_i{interval}_p{p}_r{run}.json') as f:
        data = json.load(f)
        return data
...
def main(interval=300, p=100):
    ...
    for num_nodes in nn:
        ...
        for r in range(0,10):
            try:
                data = get_result(v, num_nodes, r, interval, p)
    ...
    plt.title(f'Avg AoI & Dissemination Rate (SF) i={interval}ms, p={p/100:.2f}')

    fig.tight_layout()  # otherwise the right y-label is slightly clipped
    fig.savefig(f'./figures/sf_1_R_aoi_i{interval}_p{p}_v{v}.png', dpi=500, bbox_inches='tight', pad_inches=0.01)
    fig.savefig(f'./figures/sf_1_R_aoi_i{interval}_p{p}_v{v}.pdf', dpi=500, bbox_inches='tight', pad_inches=0.01)
    plt.show()
