...
def main(v, num_nodes, send_interval=500, run=1):
    data = pd.read_csv(f'./res/v{v}/cbf_n{int(num_nodes)}_i{int(send_interval)}_r{run}.csv')
    ...
    with open(f'./res/v{v}_parsed/cbf_n{int(num_nodes)}_i{int(send_interval)}_r{run}.json', 'w') as f:
        json.dump(data, f, indent=4, sort_keys=True)
        os.remove(f'./res/v{v}/cbf_n{int(num_nodes)}_i{int(send_interval)}_r{run}.csv')
